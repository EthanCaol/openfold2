...
if [ -f "${ROOT_DIR}/commands.txt" ]; then
    ...
    echo "| 生成下载地址 commands.txt"
    s5cmd --no-sign-request --json ls "s3://pdbsnapshots/20250101/pub/pdb/data/structures/divided/mmCIF/*/*" |
        jq -r '.key' |
